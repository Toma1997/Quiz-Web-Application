...
class MysqliDb
{
    ...
    {
        if($port == NULL)
            $port = ini_get('mysqli.default_port'); // setovanje default porta - 3306
     ...
     */
		 ...
		 // prosledjuje se naziv tabele, broj redova (po default nista), kolone (po default sve)
    public function get($tableName, $numRows = null, $columns = '*')
    {
			// ako je slucajno prazno za kolone setuj opet sve
        if (empty ($columns))
            $columns = '*';

				//ako ima vise kolona tj ako je to niz onda ih razdvoj zarezom
        $column = is_array($columns) ? implode(', ', $columns) : $columns;

				// definise upit
        $this->_query = "SELECT $column FROM $tableName";

				//izgradjuje upit za odredjen broj redova
        $stmt = $this->_buildQuery($numRows);
        $stmt->execute(); // izvsava upit
        $this->reset(); // resetuje sva polja naredbi na prazan niz
     ...
     */
     ...
     {	// preko funkcije get izvlaci samo prvi element niza od nekog upita
